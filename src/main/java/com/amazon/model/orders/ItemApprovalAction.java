/*
 * Selling Partner API for Orders
 * The Selling Partner API for Orders helps you programmatically retrieve order information. These APIs let you develop fast, flexible, custom applications in areas like order synchronization, order research, and demand-based decision support tools.
 *
 * OpenAPI spec version: v0
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */


package com.amazon.model.orders;

import java.util.Objects;
import java.util.Arrays;
import com.amazon.model.orders.ItemApprovalActionChanges;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;

/**
 * This object represents an approval action used by the actors in the order item approval process. Check the applicable restrictions at the specific approval type schemas.
 */
@ApiModel(description = "This object represents an approval action used by the actors in the order item approval process. Check the applicable restrictions at the specific approval type schemas.")
@javax.annotation.Generated(value = "io.swagger.codegen.languages.JavaClientCodegen", date = "2023-05-11T14:53:04.283+02:00")
public class ItemApprovalAction {
  /**
   * Defines the type of action for the approval.
   */
  @JsonAdapter(ActionTypeEnum.Adapter.class)
  public enum ActionTypeEnum {
    APPROVE("APPROVE"),
    
    DECLINE("DECLINE"),
    
    APPROVE_WITH_CHANGES("APPROVE_WITH_CHANGES");

    private String value;

    ActionTypeEnum(String value) {
      this.value = value;
    }

    public String getValue() {
      return value;
    }

    @Override
    public String toString() {
      return String.valueOf(value);
    }

    public static ActionTypeEnum fromValue(String text) {
      for (ActionTypeEnum b : ActionTypeEnum.values()) {
        if (String.valueOf(b.value).equals(text)) {
          return b;
        }
      }
      return null;
    }

    public static class Adapter extends TypeAdapter<ActionTypeEnum> {
      @Override
      public void write(final JsonWriter jsonWriter, final ActionTypeEnum enumeration) throws IOException {
        jsonWriter.value(enumeration.getValue());
      }

      @Override
      public ActionTypeEnum read(final JsonReader jsonReader) throws IOException {
        String value = jsonReader.nextString();
        return ActionTypeEnum.fromValue(String.valueOf(value));
      }
    }
  }

  @SerializedName("ActionType")
  private ActionTypeEnum actionType = null;

  @SerializedName("Comment")
  private String comment = null;

  @SerializedName("Changes")
  private ItemApprovalActionChanges changes = null;

  public ItemApprovalAction actionType(ActionTypeEnum actionType) {
    this.actionType = actionType;
    return this;
  }

   /**
   * Defines the type of action for the approval.
   * @return actionType
  **/
  @ApiModelProperty(required = true, value = "Defines the type of action for the approval.")
  public ActionTypeEnum getActionType() {
    return actionType;
  }

  public void setActionType(ActionTypeEnum actionType) {
    this.actionType = actionType;
  }

  public ItemApprovalAction comment(String comment) {
    this.comment = comment;
    return this;
  }

   /**
   * Comment message to provide optional additional context on the approval action.
   * @return comment
  **/
  @ApiModelProperty(value = "Comment message to provide optional additional context on the approval action.")
  public String getComment() {
    return comment;
  }

  public void setComment(String comment) {
    this.comment = comment;
  }

  public ItemApprovalAction changes(ItemApprovalActionChanges changes) {
    this.changes = changes;
    return this;
  }

   /**
   * Get changes
   * @return changes
  **/
  @ApiModelProperty(value = "")
  public ItemApprovalActionChanges getChanges() {
    return changes;
  }

  public void setChanges(ItemApprovalActionChanges changes) {
    this.changes = changes;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ItemApprovalAction itemApprovalAction = (ItemApprovalAction) o;
    return Objects.equals(this.actionType, itemApprovalAction.actionType) &&
        Objects.equals(this.comment, itemApprovalAction.comment) &&
        Objects.equals(this.changes, itemApprovalAction.changes);
  }

  @Override
  public int hashCode() {
    return Objects.hash(actionType, comment, changes);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ItemApprovalAction {\n");
    
    sb.append("    actionType: ").append(toIndentedString(actionType)).append("\n");
    sb.append("    comment: ").append(toIndentedString(comment)).append("\n");
    sb.append("    changes: ").append(toIndentedString(changes)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

